...
const aliConfig = require('config').get('aliConfig')
const logger = require('./logHelper').getLogger('MQ')
...
const getConn = async (vhostName, role, recoverFunc, args) => {
  ...
  const [err, connection] = await to(amqplib.connect(`amqp://${aliConfig.host}/${vhostName}?frameMax=0&heartbeat=5`))
...
const startConsumer = async (queue, consumeFunc) => {
  ...
  // 预加载1个消息
  await channel.prefetch(parseInt(queue.prefetch))
  // 监听并消费通知队列
